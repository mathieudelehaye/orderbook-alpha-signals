...
def _validate_dataframe(df: pd.DataFrame, required_columns: list) -> None:
    ...
    if missing_cols:
        raise SignalValidationError("Missing required columns: %s" % missing_cols)

    # Check for all-null columns
    null_cols = [col for col in required_columns if df[col].isnull().all()]
    if null_cols:
        raise SignalValidationError("Columns contain only null values: %s" % null_cols)
...
def _validate_window(window: int, min_window: int = 2) -> None:
    ...
    if not isinstance(window, int) or window < min_window:
        raise SignalValidationError(
            "Window must be integer >= %d, got %d" % (min_window, window)
        )
...
def vwap_deviation(df: pd.DataFrame, window: int = 20) -> pd.Series:
    ...
    try:
        ...
        if len(valid_signals) > 0:
            logger.debug(
                "VWAP deviation: %d valid signals, "
                "range [%.4f, %.4f]"
                % (len(valid_signals), valid_signals.min(), valid_signals.max())
            )

        return deviation

    except Exception as e:
        logger.error("Error calculating VWAP deviation: %s" % e)
...
def volatility_signal(df: pd.DataFrame, window: int = 20) -> pd.Series:
    ...
    return (vol - vol.rolling(window=window * 2).mean()) / vol.rolling(
        window=window * 2
    ).std()
...
def rsi_signal(
    df: pd.DataFrame, window: int = 14, overbought: float = 70, oversold: float = 30
) -> pd.Series:
    """
...
def combine_signals(signals: dict, weights: Optional[dict] = None) -> pd.Series:
    ...
    """
    if weights is None:
        weights = dict.fromkeys(signals.keys(), 1.0)
