    ...
    def calculate_position_size(
        ...
        try:
            ...
            if self.current_drawdown > self.max_drawdown_limit * 0.5:
                ...
                logger.warning(f"Reducing position size due to drawdown: {self.current_drawdown:.2%}")
                ...
                if current_pos != 0 and not pd.isna(entry_prices.iloc[i-1]):
                    ...
                    if current_pos > 0:  # Long position
                        ...
                        if current_price <= entry_price * (1 - self.stop_loss_pct):
                            ...
                            logger.debug(f"Long stop loss triggered at {current_price:.2f}")
                        # Take profit
                        elif current_price >= entry_price * (1 + self.take_profit_pct):
                            adjusted_positions.iloc[i] = 0
                            exit_signals.iloc[i] = 1
                            logger.debug(f"Long take profit triggered at {current_price:.2f}")
                        ...
                        if current_price >= entry_price * (1 + self.stop_loss_pct):
                            adjusted_positions.iloc[i] = 0
                            exit_signals.iloc[i] = 1
                            logger.debug(f"Short stop loss triggered at {current_price:.2f}")
                        # Take profit
                        elif current_price <= entry_price * (1 - self.take_profit_pct):
                            adjusted_positions.iloc[i] = 0
                            exit_signals.iloc[i] = -1
                            logger.debug(f"Short take profit triggered at {current_price:.2f}")
    ...
    def update_risk_state(self, current_equity: float) -> None:
        ...
        try:
            ...
            if self.current_drawdown >= self.max_drawdown_limit:
                ...
                logger.warning(f"Risk-off mode activated. Drawdown: {self.current_drawdown:.2%}")
            
            # Deactivate risk-off mode when drawdown recovers
            elif self.is_risk_off and self.current_drawdown < self.max_drawdown_limit * 0.5:
                self.is_risk_off = False
...
def apply_risk_management(
    ...
    try:
        ...
        logger.error(f"Error in risk management application: {e}")
